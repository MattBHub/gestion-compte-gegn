...
class ImportUsersCustomCommand extends CsvCommand
{

    protected function configure()
    {
        $this
            ->setName('app:import:userscustom')
        ...
        if (($handle = fopen($file, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 10000, $delimiter)) !== FALSE) {
                ...
                if ($row > 1) { //skip first line
                    ...
                    // MB change number provided for max
                    $toto = ($this->getField('member_number',$data));
                    ...
                    // pas de doublon
                    $allreadyexists = $em->getRepository(User::class)->findOneBy(array('email'=>$email));
                    if ($allreadyexists)
                        continue;
